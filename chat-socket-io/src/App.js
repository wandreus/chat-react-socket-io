...
function App() {
  ...
  const sendMessages = form => {
    ...
    if (name && Textmessages.length) {

      socket.emit('SendMessage', { name, Textmessages, entry: false });
      setMessages(array => [...array, { name, Textmessages }]);
  ...
  return (
    <form onSubmit={e => sendMessages(e)}>
      ...
      <input name="messages" placeholder="Your Message" />
